...
class SigmoidActivationFunctionTest
{
    ...
    void testActivate()
    {
        assertEquals(forCompare(0.54983), forCompare(underTest.activate(0.2)));
        assertEquals(forCompare(0.45017), forCompare(underTest.activate(-0.2d)));
    }
    ...
    {
        return new BigDecimal(value).round(new MathContext(5, RoundingMode.HALF_EVEN)).stripTrailingZeros();
    }
}
